  ...
  type?: string; labelTextColor?: string; className?: string; min?: number; max?: number; step?: number;
    ...
    <div className={className}>
        ...
        <input 
            ...
            max={max} 
            step={step}
            className={`w-full px-4 py-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl text-[13px] font-bold text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-white/20 focus:outline-none focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-inner`} 
        />
    </div>
    );
};
...
export const TextAreaField: React.FC<{ id: string; label: string; value: string; onChange: (val: string) => void; placeholder?: string; rows?: number; labelTextColor?: string; readOnly?: boolean; className?: string; }> =
    ...
    <div className={className}>
        ...
        <textarea 
            ...
            rows={rows} 
            readOnly={readOnly}
            className={`w-full px-4 py-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl text-[13px] font-bold text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-white/20 focus:outline-none focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-inner resize-none scrollbar-thin ${readOnly ? 'opacity-50 cursor-not-allowed' : ''}`} 
        />
    </div>
    );
};
  ...
  return (
    <div className={`flex items-start group cursor-pointer ${className}`} onClick={() => onChange(!checked)}>
        <div className="flex items-center h-6">
            <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${checked ? 'bg-emerald-500 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'bg-gray-100 dark:bg-white/5 border-gray-300 dark:border-white/20 group-hover:border-emerald-500/40'}`}>
                {checked && (
        ...
        <div className="ml-4 text-sm mt-0.5">
            <label htmlFor={id} className={`text-[11px] font-black uppercase tracking-widest text-gray-800 dark:text-white/80 transition-colors pointer-events-none`} style={{color: labelTextColor}} title={title}>{label}</label>
            {description && <p id={`${id}-description`} className={`text-gray-500 dark:text-white/40 text-[10px] mt-1 font-medium`}>{description}</p>}
        </div>
    </div>
  );
};
    ...
    <div className={className}>
        ...
        <div className="relative group">
            <select 
                ...
                value={value} 
                onChange={e => onChange(e.target.value)} 
                className={`w-full px-4 py-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl text-[13px] font-bold text-gray-900 dark:text-white appearance-none focus:outline-none focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-inner`}
            >
                <option disabled value="" className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white">Select Parameter...</option>
                {options.map(opt => (<option key={opt.value} value={opt.value} className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white">{opt.label}</option>))}
            </select>
