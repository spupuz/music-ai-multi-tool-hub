...
const StatDisplayCard: React.FC<StatDisplayCardProps> = ({ title, value, description, icon, children, tooltipText }) => {
  return (
    <div className="glass-card p-6 border-white/5 bg-white/5 hover:bg-white/10 hover:border-green-500/30 transition-all duration-300 min-h-[110px] flex flex-col group relative overflow-hidden">
      <div className="absolute -right-4 -top-4 w-12 h-12 bg-green-500/5 rounded-full blur-2xl group-hover:bg-green-500/10 transition-colors"></div>
      
      <div className="flex items-center gap-3 mb-4">
        {icon && (
          <div className="text-green-500/60 group-hover:text-green-400 transition-colors shrink-0 scale-90 group-hover:scale-100 duration-300">
            {icon}
          </div>
        )}
        <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-500 group-hover:text-green-500 transition-colors truncate" title={title}>
          {title}
        </h3>
        {tooltipText && (
