...
const StatChartsArea: React.FC<StatChartsAreaProps> = ({ stats, username, topNValue, onSetFilter }) => {
      ...
      <div className="space-y-6 mt-6 animate-fadeIn">
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-8">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="p-6 rounded-2xl border border-gray-200 dark:border-white/5 space-y-4">
  ...
  const fontColor = theme === 'dark' ? '#e5e7eb' : '#111827'; // gray-200 vs gray-900
  ...
  const uniqueSongsForScatterPlots = useMemo(() => {
    if (!stats) return [];

    // ⚡ Bolt: Replaced O(n^2) array.filter with findIndex with an O(n) Map approach
    // to deduplicate combined songs, and memoized it to prevent recalculation
    // and re-renders on every parent update.
    const map = new Map<string, SunoClip>();
    stats.topPlayedSongs.forEach(song => map.set(song.id, song));
    stats.topUpvotedSongs.forEach(song => map.set(song.id, song));
  ...
  switch (selectedPeriod) {
    case "sinceLastUpdate": 
      topUpvotesChartTitle = `Top Upvotes Increase (Since Last Update)`; 
      topUpvotesChartData = stats.topUpvotesIncrease; 
      topUpvotesValueLabel = "Upvote Increase"; 
      ...
      break;
    case "last7Days": 
      topUpvotesChartTitle = `Top ${topNValue} Upvotes Increase (Last 7 Days)`; 
      topUpvotesChartData = stats.topUpvotesIncrease7d; 
      topUpvotesValueLabel = "7-Day Upvote Increase"; 
      ...
      break;
    case "last30Days": 
      topUpvotesChartTitle = `Top ${topNValue} Upvotes Increase (Last 30 Days)`; 
      topUpvotesChartData = stats.topUpvotesIncrease30d; 
      topUpvotesValueLabel = "30-Day Upvote Increase"; 
      ...
      break;
    case "overall": 
      topUpvotesChartTitle = `Top ${topNValue} Most Upvoted Songs (Overall)`; 
      topUpvotesChartData = stats.topUpvotedSongs; 
      topUpvotesValueLabel = "Total Upvotes"; 
      topUpvotesBarColor = "#F472B6"; 
      topPlaysChartTitle = `Top ${topNValue} Most Played Songs (Overall)`; 
  ...
  return (
     <div className="space-y-8 mt-6">
         <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-8  ">
          <ChartContainer title="Upvotes Trend (Per Update)" tooltipText="Trend of the user's total likes over time, based on data snapshots collected by this tool at each update run."><UpvotesTrendChart data={stats.historicalUpvotes} lineColor="#ff4444" {...commonChartProps} /></ChartContainer>
      ...
      <section className="mb-10 p-8 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-3xl relative group overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
        <label htmlFor="trendPeriodSelect" className="block text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500/60 mb-4 flex items-center gap-2">
           <FilterIcon className="w-3 h-3" /> Analysis Period
        </label>
        <div className="relative inline-block w-full sm:w-auto overflow-hidden">
          <select 
            ...
            className="w-full sm:w-80 px-6 py-4 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl text-[11px] font-black uppercase tracking-widest text-gray-900 dark:text-white appearance-none focus:outline-none focus:border-emerald-500/30 transition-all cursor-pointer shadow-xl"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 mt-8  ">
        ...
        <ChartContainer title="Upvote Count Distribution" tooltipText="Shows how many songs fall into different like count brackets."><UpvoteCountDistributionChart data={stats.upvoteCountDistribution} barColor="#A78BFA" {...commonChartProps} /></ChartContainer>
      ...
      <p className="text-xs text-gray-500 mt-1 italic text-center"> Tag Pair Performance shows average metrics for songs containing specific pairs of tags (min. 3 songs per pair). </p>
        ...
        <ChartContainer title="Song Performance: Plays vs. Upvotes" heightClassName="h-80 sm:h-96 md:h-[28rem]" tooltipText="Visualizes each song by its total plays (X-axis) vs. total likes (Y-axis). The line indicates the user's overall average like rate. Helps identify high/low engagement songs."> <PlaysUpvotesScatterPlot songs={uniqueSongsForScatterPlots} averageUpvoteRateOverall={stats.overallUpvoteRate} {...commonChartProps} /> </ChartContainer>
        <ChartContainer title="Song Performance: Plays vs. Comments" heightClassName="h-80 sm:h-96 md:h-[28rem]" tooltipText="Visualizes each song by its total plays (X-axis) vs. total comments (Y-axis). The line indicates the user's overall average like rate (for songs >20 plays). Helps identify 'talkable' songs."> <PlaysCommentsScatterPlotChart songs={uniqueSongsForScatterPlots} averageCommentRateOverall={stats.overallCommentRate} {...commonChartProps} /> </ChartContainer>
      </div>
      ...
      <p className="text-[9px] font-black uppercase tracking-widest text-gray-600 text-center mt-12 py-8 border-t border-gray-200 dark:border-white/5 max-w-2xl mx-auto italic opacity-60">
        Data latency warning: snapshots are stored locally. Historical granularity is subject to Suno API constraints.
      </p>
    </div>
  );
};
