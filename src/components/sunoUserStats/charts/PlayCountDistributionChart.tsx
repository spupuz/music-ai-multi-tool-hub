...
interface PlayCountDistributionChartProps {
  data: Record<string, number>; // Key: "0-50", Value: count
  barColor?: string;
  fontColor?: string;
  gridColor?: string;
}

const PlayCountDistributionChart: React.FC<PlayCountDistributionChartProps> = ({
  data,
  ...
  useEffect(() => {
    if (chartRef.current && chartLabels.length > 0 && chartDataValues.some((v: number) => v > 0)) {
      ...
      if (ctx) {
        ...
        chartOptions.scales.x.title = { display: true, text: 'Play Count Buckets', color: fontColor, font: { size: 10 } };
  ...
  if (!chartDataValues.some(v => v > 0)) {
    return <p className="text-center text-gray-500 text-sm italic py-4">No play count distribution data available.</p>;
  }

  return <canvas ref={chartRef} aria-label="Play Count Distribution Chart"></canvas>;
};

export default PlayCountDistributionChart;
