...
const ImportExportModal: React.FC<ImportExportModalProps> = ({ show, onClose, onExport, pastedImportText, setPastedImportText, onImportPastedText, importFileRef, onFileImport }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
            <div className="glass-card p-10 max-w-lg w-full border-gray-200 dark:border-white/10 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                <div className="absolute top-0 right-0 w-48 h-48 bg-blue-500/5 blur-[80px] pointer-events-none"></div>
                
                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 dark:text-blue-500 mb-8 sticky top-0 z-10">Import/Export</h3>
                
                <div className="overflow-y-auto pr-2 custom-scrollbar flex-grow space-y-8">
                    <section>
                      <h4 className="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-4 opacity-60">Export</h4>
                      <div className="grid grid-cols-2 gap-3">
                          <Button onClick={() => onExport('txt')} variant="ghost" size="sm" className="font-black uppercase tracking-widest text-[9px] border-gray-200 dark:border-white/10"><ExportIcon className="mr-1.5 w-3 h-3" />TXT</Button>
                          <Button onClick={() => onExport('csv')} variant="ghost" size="sm" className="font-black uppercase tracking-widest text-[9px] border-gray-200 dark:border-white/10"><ExportIcon className="mr-1.5 w-3 h-3" />CSV</Button>
                      </div>
                    </section>
                    ...
                    <section className="space-y-4">
                        ...
                        <TextAreaField 
                          ...
                          className="mb-0"
                        />
                        <Button onClick={onImportPastedText} variant="primary" size="sm" className="w-full font-black uppercase tracking-widest text-[9px]" backgroundColor="#6366f1"><ImportIcon className="mr-1.5 w-3 h-3" />Import</Button>
                    </section>
                    ...
                    <section>
                        ...
                        <label
                          ...
                          className="w-full h-[42px] bg-white dark:bg-white/5 border border-dashed border-gray-200 dark:border-white/10 rounded-xl flex items-center justify-center gap-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-300 dark:hover:border-white/20 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                ...
                <div className="mt-8 pt-8 border-t border-gray-200 dark:border-white/5 sticky bottom-0 z-10">
                    <Button onClick={onClose} variant="ghost" size="sm" className="w-full font-black uppercase tracking-widest text-[9px] border-gray-200 dark:border-white/10">Abort</Button>
                </div>
            </div>
        </div>
    );
