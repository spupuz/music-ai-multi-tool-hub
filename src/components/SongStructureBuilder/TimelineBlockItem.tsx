...
export const DropIndicator: React.FC = () => (
    <div className="h-1 bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)] rounded-full my-3 opacity-100 animate-pulse" />
);
...
const TimelineBlockItem: React.FC<TimelineBlockItemProps> = ({
    ...
    return (
        <div onDragOver={(e) => onDragOver(e, index)} className="group/block relative">
            <div className="p-6 bg-white dark:bg-black/20 backdrop-blur-xl border-gray-200 dark:border-white/10 shadow-xl transition-all hover:border-gray-300 dark:hover:border-white/20 rounded-2xl border border-l-[6px] active:scale-[0.99]" style={{ borderLeftColor: blockColor }}>
                <div className="flex justify-between items-center mb-4" draggable onDragStart={(e) => onDragStart(e, 'timeline', { id: block.id, fromIndex: index })} onDragEnd={onDragEnd}>
                    <div className="flex items-center flex-grow mr-4 cursor-grab active:cursor-grabbing">
                        ...
                        />
                        <div className="flex items-center ml-4 bg-black/10 dark:bg-black/40 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-white/5 group-hover:border-gray-200 dark:group-hover:border-white/10 transition-colors">
                            <input 
                                type="number"
                                value={block.barCount || ''}
                                onChange={(e) => onBarCountChange(block.id, e.target.value)}
                                onMouseDown={(e) => e.stopPropagation()}
                                className="w-10 p-0 text-xs font-black text-center bg-transparent border-none text-emerald-600 dark:text-emerald-500 focus:ring-0"
                                placeholder="00"
                            />
                ...
                <div className="relative group/notes mb-6">
                  <textarea 
                      ...
                      rows={1} 
                      className="w-full px-4 py-3 bg-white dark:bg-black/20 border-gray-200 dark:border-white/5 focus:bg-gray-100 dark:focus:bg-white/10 border rounded-xl text-xs font-medium text-gray-700 dark:text-gray-300 placeholder-gray-500 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all resize-none overflow-hidden" 
                ...
                <div className="space-y-2 relative">
                    {block.lyrics.map((lyric, lyricIndex) => (
                        <div key={lyric.id} className="flex items-center gap-3 group/lyric bg-white dark:bg-black/10 p-1.5 rounded-xl border border-transparent hover:border-gray-200 dark:hover:border-white/5 transition-all">
                            ...
                            <div className="flex items-center shrink-0">
                              <span className="text-[9px] font-black tracking-widest text-gray-500 bg-white dark:bg-black/20 px-2.5 py-1 rounded-lg border border-gray-200 dark:border-white/5 opacity-50">
                                  {countSyllablesInLine(lyric.currentText)} SYLL
                              </span>
                            </div>
                            <div className="flex items-center opacity-0 group-hover/lyric:opacity-100 focus-within:opacity-100 transition-all gap-0.5 pr-1">
                                <Button onClick={() => onReorderLyricLine(block.id, lyricIndex, 'up')} aria-label="Move Up" disabled={lyricIndex === 0} variant="ghost" size="xs" className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 text-gray-500 disabled:opacity-20 border-none shadow-none transition-colors" title={lyricIndex === 0 ? "Cannot move up: Already at the top" : "Move Up"} startIcon={<UpArrowIcon className="w-3 h-3" />} />
                                <Button onClick={() => onReorderLyricLine(block.id, lyricIndex, 'down')} aria-label="Move Down" disabled={lyricIndex === block.lyrics.length - 1} variant="ghost" size="xs" className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 text-gray-500 disabled:opacity-20 border-none shadow-none transition-colors" title={lyricIndex === block.lyrics.length - 1 ? "Cannot move down: Already at the bottom" : "Move Down"} startIcon={<DownArrowIcon className="w-3 h-3" />} />
                                ...
                                {lyric.history.length > 0 && <Button onClick={() => onShowHistory(block.id, lyric)} aria-label="History" variant="ghost" size="xs" className="p-1.5 rounded-lg hover:bg-yellow-500/20 text-yellow-500 border-none shadow-none transition-colors" title="History" startIcon={<HistoryIcon className="w-3 h-3" />} />}
