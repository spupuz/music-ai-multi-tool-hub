...
const StructurePalette: React.FC<StructurePaletteProps> = ({
    ...
    return (
        <div className="lg:col-span-1 glass-card p-6 border-gray-200 dark:border-white/10 shadow-xl relative overflow-hidden h-fit">
            ...
            <div className="grid grid-cols-1 gap-2.5">
                {predefinedBlockTypes.map(type => {
                    ...
                    return (
                        <div key={type} className="flex items-center gap-2 p-1.5 bg-black/10 dark:bg-black/40 rounded-xl border border-gray-200 dark:border-white/5 group hover:border-gray-200 dark:hover:border-white/10 transition-all">
                            <div 
                                draggable 
            ...
            <div className="mt-8 pt-8 border-t border-gray-200 dark:border-white/5">
                ...
                <div className="flex flex-col gap-2"> 
                    <input 
                        ...
                        className="w-full px-4 py-3 bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl text-xs font-bold placeholder-gray-600 focus:ring-4 focus:ring-green-500/10 focus:border-green-500 outline-none transition-all"
                    /> 
                    <Button 
            ...
            <div className="mt-8 pt-8 border-t border-gray-200 dark:border-white/5">
                ...
                <div className="space-y-3">
                    {arrangementTemplates.map(template => (
                        <div key={template.name} className="p-4 bg-white dark:bg-black/20 rounded-2xl border border-gray-200 dark:border-white/5 hover:border-gray-200 dark:hover:border-white/10 transition-all">
                            <div className="text-[10px] font-black uppercase tracking-widest text-gray-900 dark:text-gray-200 mb-3" title={template.description}>
                                {template.name}
                            </div>
                            <div className="flex gap-2">
                              ...
                              <Button onClick={() => onApplyTemplate(template, 'append')} variant="ghost" size="xs" className="flex-grow font-black uppercase tracking-widest text-[8px] border-gray-200 dark:border-white/10">Append</Button>
                            </div>
                        </div>
                    ))}
                </div>
