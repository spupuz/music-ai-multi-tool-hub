  ...
  return (
    <div className={className}>
      ...
      <textarea
        ...
        disabled={disabled}
        readOnly={readOnly}
        className={`mt-1 block w-full px-4 py-3 rounded-xl transition-all duration-300 outline-none sm:text-sm text-gray-900 dark:text-white resize-y disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-100 dark:disabled:bg-gray-900 ${inputStyles} ${readOnly ? 'opacity-70 cursor-not-allowed bg-gray-100 dark:bg-gray-700' : ''}`}
      />
    </div>
  );
};
