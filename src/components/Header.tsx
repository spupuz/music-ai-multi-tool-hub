...
const AppLogo: React.FC = () => (
  <svg width="28" height="28" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-2 text-emerald-500">
...
const Header: React.FC<HeaderProps> = ({ onToggleSidebar, isSidebarOpen, appName }) => {
  ...
  if (nameParts.length >= 4 && nameParts[0] === "Music" && nameParts[1] === "AI") {
    styledName = (
      <>
        <span className="text-emerald-500">{nameParts[0]}</span>{' '}
        <span className="text-emerald-400/80">{nameParts[1]}</span>{' '}
        <span className="hidden min-[380px]:inline text-gray-700 dark:text-gray-200">
          {nameParts.slice(2, -1).join(' ')}{' '}
        </span>
        <span className="text-emerald-600 dark:text-emerald-400 font-bold">{nameParts.slice(-1)}</span>
      </>
    );
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 z-50 h-16 flex items-center px-2 sm:px-4 transition-[background-color,border-color,box-shadow] duration-300 glass-nav"
    >
      <div className="flex items-center px-2 flex-grow overflow-hidden">
        ...
        <div className="flex items-center flex-grow overflow-hidden">
          ...
          <h1 className="text-xs sm:text-sm font-bold whitespace-normal sm:whitespace-nowrap overflow-hidden text-ellipsis ml-1 min-[400px]:ml-2 max-w-[140px] min-[360px]:max-w-none">
            {styledName}
          </h1>
