...
const toolDescriptions: Record<ToolId, string> = {
  sunoMusicPlayer: "Dive into any Suno.com user's song library, a specific playlist, or a custom list of Suno, Flow Music, and Riffusion song URLs (one per line). Load, play, shuffle, and manage the collection with an advanced player featuring real-time visualization and a 10-band EQ. Caches data for users/playlists for faster subsequent loads. Custom URL lists are not cached.",
  ...
  sunoSongCompliance: "Verify Suno, Riffusion, and Flow Music song titles against contest rules (editable pattern) and analyze lyrics using Google Gemini API for language ID (plus untranslatable word detection) and content rating (G, PG, PG-13, R, Explicit). Features batch URL processing, CSV export, save/load URL lists, and a link to the Lyric Processor for problematic lyrics.",
  ...
  bpmTapper: "Quickly find the Beats Per Minute (BPM) of any song by tapping along to the rhythm. Also includes a Musical Key Finder where you can upload an audio file or provide a Suno/Riffusion/Flow Music URL to analyze and suggest the song's key and BPM.",
  ...
  return (
    ...
    >
      <div className="flex items-center mb-4">
        {icon && <span className="mr-4 flex-shrink-0 text-emerald-500">{React.cloneElement(icon as React.ReactElement<{ className?: string }>, { className: "w-8 h-8" })}</span>}
...
[Your Name / Discord Handle (Optional)]`;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto glass-card p-4 sm:p-8 md:p-12 border-white/10 shadow-2xl transition-[box-shadow,border-color] duration-500 animate-fadeIn relative overflow-hidden">
      ...
      <main className="text-gray-700 dark:text-gray-300 leading-relaxed">
        <section id="mission">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-10 mb-6 border-b border-gray-200 dark:border-white/10 pb-3 text-left">Our Mission & Ethos</h2>
          <div className="bg-white/5 p-6 sm:p-8 rounded-2xl border border-white/10 shadow-inner">
            ...
            <p className="mb-3 leading-relaxed text-gray-700 dark:text-gray-300 text-lg mt-4 font-bold opacity-80">
              <strong className="font-black text-emerald-600 dark:text-emerald-400">Community First:</strong> This Hub is a community-driven project, created for the love of music and AI exploration, <strong className="font-black text-emerald-600 dark:text-emerald-400">not for financial gain</strong>. Any songs or creative works featured within our tools (such as the Music Shuffler, Song Deck Picker, or SparkTune Challenge Generator) are presented as links that direct you to their original source pages (e.g., Suno.com, FlowMusic.app). We strive to always link back to the original creators and their profiles, ensuring that all credit for these amazing works rightfully goes to them. This platform is about sharing, learning, and creating together.
            </p>
          </div>
        </section>

        <section id="workflows">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-10 mb-6 border-b border-gray-200 dark:border-white/10 pb-3 text-left">Quick Start Workflows</h2>
              ...
              return (
                <div key={workflow.title} className="bg-white/5 p-6 rounded-2xl border border-white/10 flex flex-col">
                  <div className="flex items-center mb-3">
                    ...
                    <h4 className="text-lg font-bold text-gray-900 dark:text-white">{workflow.title}</h4>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10 flex flex-wrap gap-2">
                    {workflow.actions.map(action => (
                      <button
                        ...
                        className="py-1.5 px-3 bg-white/5 hover:bg-emerald-500/20 hover:text-white text-gray-600 dark:text-gray-400 rounded-lg text-xs font-medium border border-white/5 transition-all"
                      >
                        {action.label}
                      </button>
                    ))}
        ...
        <section id="support-the-hub" className="mt-12">
          <div className="bg-emerald-500/5 p-8 rounded-2xl border border-white/10 group relative overflow-hidden">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6 text-center">Support the Hub's Journey!</h3>
            <div className="flex flex-col items-center">
              <div className="flex flex-col items-center gap-6 mb-8 w-full max-w-xl relative z-10">
                <div className="w-full bg-white/5 p-8 rounded-2xl text-center flex flex-col items-center justify-between border-2 border-emerald-500/50 hover:border-emerald-500/80 transition-all shadow-xl hover:-translate-y-1 duration-300">
                  <div>
                    <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-3 leading-tight">Direct Contribution</h4>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left max-w-3xl w-full mt-6 bg-white/5 p-6 rounded-2xl border border-white/10">
                ...
                <div className="flex gap-3">
                  <div className="w-1 h-1 rounded-full bg-emerald-500 mt-2 flex-shrink-0"></div>
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">Explore advanced creative possibilities.</p>
                </div>
                <div className="flex gap-3">
                  <div className="w-1 h-1 rounded-full bg-emerald-500 mt-2 flex-shrink-0"></div>
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">Keep the Hub thriving as a global resource.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="tools">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-12 mb-8 border-b border-gray-200 dark:border-white/10 pb-3 text-left">Meet the Tools</h2>
          {categoryDisplayOrder.map(categoryName => {
            ...
            return (
              <div key={categoryName} className="mb-12">
                <h3 className="text-base font-bold text-gray-500 dark:text-gray-400 mt-8 mb-6 border-b border-gray-200 dark:border-white/10 pb-2">{categoryName}</h3>
        ...
        <section id="story">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-12 mb-6 border-b border-gray-200 dark:border-white/10 pb-3 text-left">The Story & Team</h2>
        ...
        <section id="tech-stack">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-12 mb-6 border-b border-gray-200 dark:border-white/10 pb-3 text-left">Tech Highlights</h2>
          <div className="bg-white/5 p-8 rounded-2xl border border-white/10 shadow-inner">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <li className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Core Architecture</span>
              ...
              <li className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Styling System</span>
              ...
              <li className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Edge Processing</span>
              ...
              <li className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Secure Privacy</span>
        ...
        <section id="feedback">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-12 mb-6 border-b border-gray-200 dark:border-white/10 pb-3 text-left">Feedback</h2>
        ...
        <section id="privacy-policy">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-12 mb-6 border-b border-gray-200 dark:border-white/10 pb-3 text-left">Privacy</h2>
          <div className="bg-white/5 p-8 rounded-2xl border border-white/10">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-6">Last Updated: {privacyPolicyLastUpdated}</p>

            <p className="mb-4 text-lg font-bold text-gray-900 dark:text-white">Your Data Stays Yours.</p>
            ...
            <h4 className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 mb-4">Infrastructure Details</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
               <div className="bg-white/5 p-6 rounded-xl border border-white/5">
                  <p className="text-sm font-bold text-gray-900 dark:text-white mb-2">Local Storage</p>
               ...
               <div className="bg-white/5 p-6 rounded-xl border border-white/5">
                  <p className="text-sm font-bold text-gray-900 dark:text-white mb-2">Relays</p>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-10 text-center">Agreement implicitly valid by application usage.</p>
          </div>
        </section>
      </main>
