...
const TimeInput: React.FC<{ label: string; value: string; onChange: (val: string) => void; id: string }> = ({ label, value, onChange, id }) => (
    <div>
        ...
        <input
            ...
            className="mt-1 block w-full px-2 py-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 text-sm text-gray-900 dark:text-white"
  ...
  return (
    <div className={`w-full ${uiMode === 'classic' ? 'max-w-7xl mx-auto px-4 pb-20' : ''}`}>
      ...
      <main className="w-full glass-card p-2 sm:p-6 md:p-10 border-white/10 text-gray-900 dark:text-gray-200 transition-all duration-500 animate-fadeIn overflow-hidden">
          ...
          <div className="flex flex-row items-center justify-center gap-6 p-4 bg-white/5 dark:bg-black/20 rounded-2xl border border-white/5">
            ...
            {fileName && (
              <div className="text-left flex-1 min-w-0">
                ...
                <p className="text-sm font-black text-emerald-600 dark:text-emerald-300 truncate max-w-[150px] sm:max-w-[400px]" title={fileName}>{fileName}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {isLoading && (
          <div className="flex flex-row items-center justify-center gap-6 my-6 p-4 bg-emerald-500/5 rounded-2xl animate-pulse">
            <Spinner color="text-emerald-600" />
            <div className="text-left">
              <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Processing Audio</p>
              <p className="text-xs font-bold text-emerald-600/60 uppercase tracking-tighter">{urlLoadingProgress || 'Hydrating stream...'}</p>
            </div>
          </div>
        )}
        ...
        {sunoArtistName && (
          <div className="my-4 p-3 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col items-center gap-3">
            <img 
              ...
              className="w-32 h-32 md:w-40 md:h-40 object-cover rounded-md border-2 border-emerald-500" 
              onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = FALLBACK_IMAGE_DATA_URI; }}
            />
            {sunoArtistName && <p className="text-md text-emerald-700 dark:text-emerald-300">Artist: {sunoArtistName}</p>}
            {sunoCoverArtUrl && (
              <Button
          ...
          <>
            ...
            <div className="flex items-center justify-center gap-2 mb-6 text-sm">
                ...
                <input 
                    ...
                    className="w-32 h-2 bg-gray-200 dark:bg-gray-600 rounded-lg appearance-none cursor-pointer accent-emerald-500 dark:accent-emerald-400"
