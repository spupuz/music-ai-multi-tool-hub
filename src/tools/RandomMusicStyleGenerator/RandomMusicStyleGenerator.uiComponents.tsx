...
export const CategoryItemDisplay: React.FC<CategoryItemDisplayProps> = ({
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 bg-white dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/5 ${effectiveCardDisabled ? 'opacity-40 grayscale pointer-events-none' : 'hover:border-gray-200 dark:hover:border-white/10 hover:bg-white-[0.07] shadow-xl'} transition-all group relative overflow-hidden gap-3 sm:gap-4`}>
        ...
        <span className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-500/80 mb-1.5 flex items-center">
          ...
          {!sunoPromptMode && (
            <Button 
              ...
              variant="ghost" 
              size="xs" 
              className="ml-3 p-1 rounded-full bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-emerald-500 hover:bg-emerald-500 hover:text-black transition-all"
          ...
          <div className="flex flex-wrap items-center mt-1">
            {displayValues.map((val, index) => (
              <span key={index} className="text-gray-900 dark:text-white text-[12px] sm:text-[13px] font-bold tracking-tight py-0.5 px-0 mr-1.5 flex items-center">
                {val}
                {isItemCustom(categoryKey as CustomItemCategoryKey, val) && <UserIcon className="inline-block ml-2 w-3 h-3 text-emerald-400 opacity-80" title="Custom"/>}
                {index < displayValues.length - 1 && <span className="opacity-30 ml-1.5">/</span>}
              </span>
            ))}
          </div>
        ) : (
          <span className="text-white/30 text-[11px] font-bold tracking-tight mt-1 italic block">(None)</span>
        )}
      </div>
      {!sunoPromptMode && (
        <div className="flex items-center justify-end w-full sm:w-auto space-x-2 z-10 pt-2 sm:pt-0 border-t sm:border-none border-gray-200 dark:border-white/5">
          {isMultiSelectCategory && onSetCategoryIntensity && currentIntensity && (
           ...
           <Button 
            ...
            variant="ghost"
            size="sm"
            className="w-9 h-9 p-0 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-500 hover:text-emerald-500 hover:border-emerald-500/30 transition-all border-none shadow-none"
          ...
          <Button 
            ...
            variant="ghost"
            size="sm"
            className={`w-9 h-9 p-0 rounded-xl transition-all border-none ${isLocked && isOptionalCategoryOn ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20 hover:bg-emerald-400' : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-500 hover:text-gray-900 dark:hover:text-white'}`} 
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
            <div className="glass-card p-8 border-emerald-500/30 shadow-2xl w-full max-w-md relative overflow-hidden">
                ...
                <div className="flex gap-4"> 
                    <Button onClick={onClose} variant="ghost" className="flex-1 py-4 h-auto rounded-2xl font-black uppercase tracking-widest text-[10px] border-gray-200 dark:border-white/10 text-gray-400 hover:text-gray-900 dark:hover:text-white">Cancel</Button> 
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
            <div className="glass-card p-0 border-emerald-500/30 shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col relative overflow-hidden">
                ...
                <header className="p-8 pb-4">
                    <h3 className="text-[12px] font-black uppercase tracking-[0.4em] text-emerald-500 px-1">Custom Items</h3>
                </header>
                ...
                <div className="overflow-y-auto px-8 pb-8 pt-2 scrollbar-thin scrollbar-thumb-emerald-500/20 scrollbar-track-transparent flex-grow">
                    {customItemEntries.filter(([_, items]) => items.length > 0).length === 0 ? ( 
                        <div className="py-12 text-center">
                            <p className="text-gray-400 dark:text-white/20 text-xs font-black uppercase tracking-widest italic">No custom items yet</p> 
                        </div>
                    ) : (
                        customItemEntries.map(([catKey, items]) => items.length > 0 && (
                            <div key={catKey} className="mb-8 last:mb-0">
                                ...
                                <div className="space-y-2"> 
                                    {items.map(item => ( 
                                        <div key={item} className="p-4 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/5 rounded-2xl flex justify-between items-center group hover:border-gray-200 dark:hover:border-white/10 transition-all"> 
...
export const ImportConfirmationModal: React.FC<{
    ...
    return (
         <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[110] p-4 animate-in fade-in duration-300">
            <div className="glass-card p-8 border-emerald-500/30 shadow-2xl w-full max-w-md relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-emerald-500/50"></div>
                <h3 className="text-[12px] font-black uppercase tracking-[0.4em] text-emerald-500 mb-6 px-1 text-center">Configuration Merge</h3>
                <p className="text-gray-700 dark:text-white/60 mb-8 text-[11px] font-bold text-center leading-relaxed">System has detected a valid parameter set. Choose integration protocol:</p>
                <div className="flex flex-col gap-4">
                    ...
                    <Button onClick={onClose} variant="ghost" className="py-4 h-auto rounded-2xl font-black uppercase tracking-widest text-[10px] border-gray-200 dark:border-white/10 text-gray-400 hover:text-gray-900 dark:hover:text-white">Cancel</Button>
                </div>
            </div>
        </div>
    );
