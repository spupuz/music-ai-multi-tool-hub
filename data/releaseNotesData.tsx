...
export const releaseNotes: ReleaseNoteItem[] = [
  {
    ...
    content: (
      <section id="version-2.0.0">
        <SectionTitle>Version 2.0.0 - Cloudflare Migration & Open Source Release</SectionTitle>
        <P><STRONG>This major update transitions the entire infrastructure to a secure, serverless architecture on Cloudflare and marks the official public release of the repository.</STRONG></P>

        <SubSectionTitle>Architecture Overhaul</SubSectionTitle>
        <UL>
          <LI><STRONG>Serverless Deployment:</STRONG> Migrated from Docker/Nginx to Cloudflare Pages for faster, more reliable global delivery.</LI>
          <LI><STRONG>Gemini Proxy Worker:</STRONG> Implemented a dedicated Cloudflare Worker to handle all AI requests, ensuring the Gemini API key remains 100% secure and hidden from the client browser.</LI>
          <LI><STRONG>Secure Password Verification:</STRONG> The Suno Song Compliance password is now verified server-side on the Worker, eliminating secrets from the frontend bundle.</LI>
        </UL>

        <SubSectionTitle>Security & Open Source</SubSectionTitle>
        <UL>
          <LI><STRONG>Git History Scrub:</STRONG> Performed a deep-clean of the repository history to remove legacy secrets, enabling a safe public launch.</LI>
          <LI><STRONG>Infrastructure Cleanup:</STRONG> Removed all obsolete Docker and Watchtower files to simplify the codebase for contributors.</LI>
        </UL>

        <SubSectionTitle>Enhanced Documentation</SubSectionTitle>
        <UL>
          <LI>Completely updated <CODE>README.md</CODE>, <CODE>DEPLOYMENT.md</CODE>, and <CODE>CONTEXT.md</CODE> to reflect the new stack and provide clear onboarding for new developers.</LI>
        </UL>
      </section>
    )
  },
