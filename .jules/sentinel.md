...
## 2025-02-28 - Missing Rate Limiting on Unauthenticated KV Endpoints
**Vulnerability:** The `/stats` and `/telemetry` proxy worker endpoints were missing rate limiting, allowing an attacker to spoof Cloudflare connecting IPs and spam requests, potentially exhausting the free-tier KV write/read quotas (Denial-of-Wallet) and distorting application telemetry metrics.
**Learning:** Cloudflare Workers exposing unauthenticated routes that interact with billing-constrained storage mechanisms (like Workers KV) must explicitly implement quota-protective rate limits. Although KV has a 1-write-per-second-per-key soft limit, unmetered access can quickly drain daily read/write limits on hobbyist projects.
**Prevention:** Ensure all external-facing endpoints utilizing backend resources apply IP-based rate limiting via TTL-bound KV entries to mitigate high-frequency abuse.
...
## 2025-03-04 - [HIGH] React href XSS via javascript: URIs in LocalMusicResourceDirectoryTool
**Vulnerability:** Found unsanitized `href` binding for `item.url` in `LocalMusicResourceDirectoryTool.tsx`. If an attacker were able to control this data source (e.g. by manipulating the initial resources if they ever become dynamic/user-provided), they could provide a URL like `javascript:alert(1)` which React would render as-is, executing arbitrary code upon click.
**Learning:** Even statically defined or pseudo-static URLs should be wrapped in `sanitizeUrlForHref` if there's any chance they will ever be sourced externally, imported, or otherwise modified, especially since `javascript:` and `data:` URIs are not inherently sanitized by React's `href` attribute binding.
**Prevention:** Always wrap URL variables with `sanitizeUrlForHref` (or a similar protocol-checking utility) before binding them to `href` in JSX, especially in directory components or lists that render links.
