...
## 2026-09-08 - [Explanatory titles for disabled buttons]\n**Learning:** Users can be confused when a button is disabled without clear feedback as to why, such as when they attempt to export an empty queue or load themes when none exist. Providing explanatory titles enhances UX and accessibility.\n**Action:** When a button's disabled state is bound to a dynamic condition, provide a corresponding conditionally rendered `title` attribute to explain the specific reason to the user.
