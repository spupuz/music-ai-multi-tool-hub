...
interface CommentCountDistributionChartProps {
...
const CommentCountDistributionChart: React.FC<CommentCountDistributionChartProps> = ({
  data,
  barColor = '#FFECB3', // Light Yellow
  fontColor = '#e5e7eb',
  gridColor = '#374151',
}) => {
  ...
  useEffect(() => {
    if (chartRef.current && chartLabels.length > 0 && chartDataValues.some((v: number) => v > 0)) {
      ...
      if (ctx) {
        ...
        chartOptions.scales.x.title = { display: true, text: 'Comment Count Buckets', color: fontColor, font: { size: 10 } };
        ...
        chartInstanceRef.current = new Chart(ctx, {
          ...
          data: {
            labels: chartLabels,
            datasets: [{
              label: 'Songs in Comment Bracket',
  ...
  if (!chartDataValues.some(v => v > 0)) {
    return <p className="text-center text-gray-500 text-sm italic py-4">No comment count distribution data available.</p>;
  }

  return <canvas ref={chartRef} aria-label="Comment Count Distribution Chart"></canvas>;
};

export default CommentCountDistributionChart;
