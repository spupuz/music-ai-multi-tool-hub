...
interface TopEngagingSongsChartProps {
...
const TopEngagingSongsChart: React.FC<TopEngagingSongsChartProps> = ({
  data,
      ...
      if (ctx) {
        ...
        chartOptions.plugins.tooltip.callbacks = {
            ...
            label: function(context: any) {
                const ratio = processedData[context.dataIndex]?.upvoteRate;
                const plays = processedData[context.dataIndex]?.song.play_count;
                const upvotes = processedData[context.dataIndex]?.song.upvote_count;
                let label = ` Upvote Rate: ${ratio !== undefined ? ratio.toFixed(1) : 'N/A'}%`;
                if (plays !== undefined) label += ` (P: ${plays.toLocaleString()})`;
                if (upvotes !== undefined) label += ` (U: ${upvotes.toLocaleString()})`;
                return label;
            }
        };
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="flex-grow w-full relative min-h-0">
        <canvas ref={chartRef} aria-label="Top Engaging Songs Chart"></canvas>
      </div>
...
export default TopEngagingSongsChart;
