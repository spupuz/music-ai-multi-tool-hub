...
interface UpvotesTrendChartProps {
...
const UpvotesTrendChart: React.FC<UpvotesTrendChartProps> = ({ 
  data, 
  lineColor = '#ff4444', // Default Red
  fontColor = '#e5e7eb',
  gridColor = '#374151',
}) => {
    ...
    if (chartRef.current && data && data.length > 0) { 
      ...
      if (ctx) {
        ...
        if (values.length > 0) {
          ...
          if (values.length === 1) {
            const padding = Math.max(2, Math.abs(minVal * 0.1) || 5); // Ensure some padding, default 5 if 10% is too small
            yMinCalculated = Math.max(0, minVal - padding);
            yMaxCalculated = maxVal + padding;
            if (yMinCalculated >= yMaxCalculated) yMaxCalculated = yMinCalculated + 10; // Ensure visible range
          } else {
  ...
  return <canvas ref={chartRef} aria-label="Upvotes Trend Chart"></canvas>;
};

export default UpvotesTrendChart;